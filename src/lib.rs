...
mod tests {
    ...
    fn generate_correct_isin_check_digit() {
        assert_eq!(
            calculate_isin_check_digit(String::from("US0378331005")),
            Some(5)
        );
        assert_eq!(
            calculate_isin_check_digit(String::from("US037833100")),
            Some(5)
        );
        assert_eq!(
            calculate_isin_check_digit(String::from("AU0000XVGZA3")),
            Some(3)
        );
        assert_eq!(
            calculate_isin_check_digit(String::from("AU0000XVGZA")),
            Some(3)
        );
        assert_eq!(
            calculate_isin_check_digit(String::from("GB00BH4HKS39")),
            Some(9)
        );
        assert_eq!(
            calculate_isin_check_digit(String::from("GB0002634946")),
            Some(6)
        );
        assert_eq!(
            calculate_isin_check_digit(String::from("US037#331005")),
            None
        );
        assert_eq!(calculate_isin_check_digit(String::from("")), None);
        assert_eq!(calculate_isin_check_digit(String::from("Test")), None);
        assert_eq!(
            calculate_isin_check_digit(String::from("TestTestTestTest")),
            None
        );
    }
    ...
    fn generate_correct_cusip_check_digit() {
        assert_eq!(
            calculate_cusip_check_digit(String::from("037833100")),
            Some(0)
        );
        assert_eq!(
            calculate_cusip_check_digit(String::from("03783310")),
            Some(0)
        );
        assert_eq!(
            calculate_cusip_check_digit(String::from("17275R102")),
            Some(2)
        );
        assert_eq!(
            calculate_cusip_check_digit(String::from("17275R10")),
            Some(2)
        );
        assert_eq!(
            calculate_cusip_check_digit(String::from("38259P508")),
            Some(8)
        );
        assert_eq!(
            calculate_cusip_check_digit(String::from("594918104")),
            Some(4)
        );
        assert_eq!(
            calculate_cusip_check_digit(String::from("68389X105")),
            Some(5)
        );
        assert_eq!(calculate_cusip_check_digit(String::from("68389$105")), None);
        assert_eq!(calculate_cusip_check_digit(String::from("")), None);
        assert_eq!(calculate_cusip_check_digit(String::from("Test")), None);
        assert_eq!(
            calculate_cusip_check_digit(String::from("TestTestTest")),
            None
        );
    }
    ...
    fn generate_correct_sedol_check_digit() {
        assert_eq!(
            calculate_sedol_check_digit(String::from("0263494")),
            Some(4)
        );
        assert_eq!(calculate_sedol_check_digit(String::from("026349")), Some(4));
        assert_eq!(
            calculate_sedol_check_digit(String::from("B09LQS3")),
            Some(3)
        );
        assert_eq!(calculate_sedol_check_digit(String::from("B09LQS")), Some(3));
        assert_eq!(
            calculate_sedol_check_digit(String::from("B677469")),
            Some(9)
        );
        assert_eq!(
            calculate_sedol_check_digit(String::from("0981536")),
            Some(6)
        );
        assert_eq!(calculate_sedol_check_digit(String::from("A023494")), None);
        assert_eq!(calculate_sedol_check_digit(String::from("")), None);
        assert_eq!(calculate_sedol_check_digit(String::from("Test")), None);
        assert_eq!(calculate_sedol_check_digit(String::from("TestTest")), None);
    }

    #[test]
    fn generate_correct_figi_check_digit() {
        assert_eq!(
            calculate_figi_check_digit(String::from("BBG000BLNQ16")),
            Some(6)
        );
        assert_eq!(
            calculate_figi_check_digit(String::from("BBG000BLNQ1")),
            Some(6)
        );
        assert_eq!(calculate_figi_check_digit(String::from("")), None);
        assert_eq!(calculate_figi_check_digit(String::from("Test")), None);
        assert_eq!(calculate_figi_check_digit(String::from("TestTest")), None);
    }

    #[test]
    fn generate_isin_from_sedol() {
        assert_eq!(
            convert_sedol_to_gb_isin(String::from("B09LQS3")),
            Some(String::from("GB00B09LQS34"))
        );
    ...
    fn generate_isin_from_cusip() {
        assert_eq!(
            convert_cusip_to_us_isin(String::from("037833100")),
...
pub fn calculate_isin_check_digit(isin: String) -> Option<u8> {
    if isin.len() < 11 || isin.len() > 12 {
        return None;
    }

    let last_char_index = 11;
...
pub fn calculate_figi_check_digit(figi: String) -> Option<u8> {
    ...
    let digit_string = figi_digits
        .iter()
        .map(|x| x.to_string())
        .collect::<Vec<String>>()
        .concat();
    let digit_chars: Vec<char> = digit_string.chars().collect();
    let digits: Vec<u32> = digit_chars
        .iter()
...
fn convert_sedol_to_isin(sedol: String, prefix: String) -> Option<String> {
    ...
    match calculate_sedol_check_digit(sedol) {
        Some(_) => match calculate_isin_check_digit(format!("{}?", &isin_prefix)) {
